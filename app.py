...
def get_location_tier(location_name: str) -> str:
    ...
    for tier, locs in LOCATION_TIERS.items():
        for loc in locs:
            if loc.lower() == location_name.strip().lower():
                return tier
    return 'budget'
...
def predict_price():
    try:
        ...
        location_multiplier = TIER_MULTIPLIERS.get(tier, 1.0)

        # Apply age depreciation
        age = int(data.get("property_age", 0) or 0)
        age_factor = calculate_age_factor(age)

        final_price = base_price * location_multiplier * age_factor

        print(f"💰 Base Price: {base_price:.2f}")
        print(f"📍 Location Tier: {tier} (x{location_multiplier})")
        print(f"🏗️ Age Factor: {age_factor:.2f}")
